...
saveRDS(rforest.model, "./rf_predict_sub_categories.rds")
super_model_rf <- readRDS("./rf_predict_sub_categories.rds")
predict.rforest = as.data.frame(h2o.predict(super_model_rf, test.h2o[-1]))
#h2o.confusionMatrix(predict.rforest[,1], test.h2o[,1])
confusionMatrix(as.matrix(predict.rforest$predict),as.matrix(test.h2o$sub_categories))
